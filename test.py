...
def find_number(cnts, binImg, imgtemp):
    ...
    for c in cnts:
        ...
        if h / w > 1.5 and h / w < 4 and h >= hf or cv2.contourArea(c) > 4600 and h <= hl: 
            ...
            # Chuyển thành np để tí xài tạo thành mảng numpy
            curr_num = np.array(curr_num, dtype=np.float32)
            # Reshape lại để tự sắp xếp thành hàng ngang
...
def findLP_img(OriImg): 
    ...
    for (x, y, w, h) in plates:
        ...
        plate_num = find_number(contours_detect(Pretreatment(img)), Pretreatment(img), img)
